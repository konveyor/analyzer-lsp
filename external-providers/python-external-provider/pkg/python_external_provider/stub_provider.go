...
// StubProvider is a Step 1 scaffold: satisfies provider.BaseClient until pylsp
// code is copied in (implementation plan Step 3).
...
func (s *StubProvider) Init(ctx context.Context, log logr.Logger, cfg provider.InitConfig) (provider.ServiceClient, provider.InitConfig, error) {
	...
	_ = log
	_ = cfg
	return nil, provider.InitConfig{}, fmt.Errorf("python-external-provider: scaffold only; Init not implemented (see implementation plan Step 3)")
}
