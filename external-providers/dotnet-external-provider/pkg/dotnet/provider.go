...
func (p *dotnetProvider) Init(ctx context.Context, log logr.Logger, config provider.InitConfig) (provider.ServiceClient, provider.InitConfig, error) {
	...
	if mode != provider.SourceOnlyAnalysisMode {
		return nil, provider.InitConfig{}, fmt.Errorf("only source analysis is supported")
	}
...
func (d *dotnetProvider) Prepare(ctx context.Context, conditionsByCap []provider.ConditionsByCap) error {
	return nil
}
